...
DateInput.prototype = {
  build: function() {
    var monthNav = $('<p class="month-nav">' +
      '<span class="c-button c-prev"></span>' +
      ' <span class="month-name"></span> ' +
      '<span class="c-button c-next"></span>' +
      '</p>');
    this.monthNameSpan = $(".month-name", monthNav);
    $(".c-prev", monthNav).click(this.bindToObj(function() { this.moveMonthBy(-1); }));
    $(".c-next", monthNav).click(this.bindToObj(function() { this.moveMonthBy(1); }));
    ...
    $(".c-prev", yearNav).click(this.bindToObj(function() { this.moveMonthBy(-12); }));
    $(".c-next", yearNav).click(this.bindToObj(function() { this.moveMonthBy(12); }));
    
    var nav = $('<div class="c-nav"></div>').append(monthNav, yearNav);
    ...
    this.dateSelector = this.rootLayers = $('<div class="date-selector"></div>').append(nav, tableShell).insertAfter(this.input);
    ...
    if ($.browser.msie && $.browser.version < 7) {
      ...
      this.ieframe = $('<iframe class="date-selector_ieframe" frameborder="0" src="#"></iframe>').insertBefore(this.dateSelector);
      this.rootLayers = this.rootLayers.add(this.ieframe);
      
      // IE 6 only does :hover on A elements
      $(".c-button", nav).mouseover(function() { $(this).addClass("hover") });
      $(".c-button", nav).mouseout(function() { $(this).removeClass("hover") });
    };
    ...
    if (!this.currentMonth || !(this.currentMonth.getFullYear() == newMonth.getFullYear() &&
      ...
      for (var i = 0; i <= numDays; i++) {
        ...
        if (currentDay.getMonth() == date.getMonth()) {
          dayCells += '<td class="selectable-day" date="' + this.dateToString(currentDay) + '">' + currentDay.getDate() + '</td>';
        } else {
          dayCells += '<td class="unselected-month" date="' + this.dateToString(currentDay) + '">' + currentDay.getDate() + '</td>';
        };
...
$(document).ready(function(){	
	$(".settings__main_calendar").live("mousedown",function(){
		$(this).addClass("active");
		$(".date-selector").css("display","block");
	})
	$(".selectable-day").live("click",function(){
		$(".date-selector").css("display","none");
		$(".settings__main_calendar").removeClass("active");
	})
	...
	$('body').click(function(e) {
		var target = $(e.target);
		if(target.is('.settings__main_calendar')||target.is('.settings__main_calendar *')||target.is('.date-selector')||target.is('.date-selector *')){return}
		else{
			$(".date-selector").css("display","none");
			$(".settings__main_calendar").removeClass("active");				
		}
